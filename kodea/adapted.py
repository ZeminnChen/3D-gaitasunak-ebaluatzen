...
class VisionEncoder(nn.Module):
    ...
    def _build_registry(self):
        return {
            ...
            "siglip2": {
                #"base": "google/siglip2-base-patch16-224", #google/siglip-so400m-patch14-384
            ...
            "pe": {
                "base": "PE-Core-B16-224", # PE-Core-B16-224, PE-Core-L14-336
    ...
    def _load_model(self):
        ...
        if self.model_type == "pe":
            ...
            # pooling: Tanh + Linear aplikatu CLS gainean
...
class Reassemble(nn.Module):
    def __init__(self, in_dim, out_dim=256, model_type=None):
        ...
        self.proj_in = nn.Sequential(nn.Linear(2 * in_dim, in_dim), nn.SiLU())
...
class DPT3DModel(nn.Module):
    def __init__(self, encoder, model_type):
        ...
        self.reassemble = nn.ModuleList([Reassemble(encoder.embed_dim, 256, model_type) for _ in range(4)])
...
def build_num_objects_map(scenes_json_path):
    with open(scenes_json_path, 'r') as f:
        data = json.load(f)
    # {image_index: num_objects}
    return {scene["image_index"]: len(scene["objects"]) for scene in data["scenes"]}

def test(dataloader, decoder, model_type, size, device, metric, save_predictions, save_dir, num_objects_map=None):
    decoder.eval()
    metric.reset()

    all_preds = []
    per_obj_metrics = {}  # {n_objects: BinaryJaccardIndex}
    ...
    if save_predictions:
        ...
        np.save(os.path.join(save_dir, f"test_{model_type}_{size}_{decoder.encoder.patch_size}.npy"), all_preds[:25].numpy())

    global_miou = metric.compute().item()
    print(f"\nTest mIoU global: {global_miou:.4f}")
...
def train_and_validate(train_loader, val_loader, decoder, model_type, size, optimizer, criterion, metric, epochs, device, model_path):
    best_dev_miou = 0.0
    ...
    for epoch in range(epochs):
        ...
        dev_miou = test(val_loader, decoder, model_type, size, metric, device, False, None)
        print(f"Dev IoU: {dev_miou:.4f}")
...
if __name__ == "__main__":
    model_type = "mae"  # "clip", "siglip2", "dinov2"
    size = "base"
    input_size = 224
    ...
    # 2. Decoder
    decoder = DPT3DModel(encoder, model_type).to(device)
    ...
    model_path = "/home/zchen002/TFG/trained_models/voxel=4/mae_base_16.pth"
    print(f"Starting training for {model_type.upper()}, epochs: {EPOCHS}, batch size: {BATCH_SIZE}, learning rate: {LR}")
    # train_and_validate(train_loader, val_loader, decoder, model_type, size, optimizer, criterion, metric, EPOCHS, device, model_path)
    
    # 6. Test
    decoder.load_state_dict(torch.load(model_path, map_location=device))
    predictions_path = f"/home/zchen002/TFG/iragarpenak/voxel=4"
    os.makedirs(predictions_path, exist_ok=True)
    num_objects_map = build_num_objects_map("/home/zchen002/TFG/CLEVR_images/scenes/CLEVR_val_scenes.json")
    test(test_loader, decoder, model_type, size, device, metric, True, predictions_path, num_objects_map)
