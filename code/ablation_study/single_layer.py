...
class VisionEncoder(nn.Module):
    ...
    def _load_model(self):
        ...
        print(f"Loaded {self.model_type.upper()} {self.size} with embed_dim={embed_dim}, patch_size={patch_size} (Extracting final layer only)")
...
class DPT3DModel(nn.Module):
    def __init__(self, embed_dim, model_type):  
        super().__init__()
        self.reassemble = Reassemble(in_dim=embed_dim, out_dim=256, model_type=model_type)
...
def get_dataloader(indices, input_size, mode, batch_size, processor, model_type):
    ...
    if model_type in ["clip", "siglip2"]:
        full_transform = lambda img: processor(images=padding(img), return_tensors="pt").pixel_values.squeeze(0)
    else:
    ...
    return DataLoader(dataset, batch_size=batch_size, shuffle=(mode == "train"), num_workers=4, pin_memory=True)
...
def train(dataloader, encoder, decoder, criterion, optimizer, metric, device):
    ...
    for images, voxels in dataloader:
        ...
        final_layer = encoder(images)   
        pred = decoder(final_layer)     
...
def train_and_validate(train_loader, val_loader, encoder, decoder, model_type, size, optimizer, criterion, metric, epochs, device, model_path):
    ...
    for epoch in range(epochs):
        ...
        dev_loss, dev_miou = test(
            dataloader=val_loader, encoder=encoder, decoder=decoder, 
            model_type=model_type, size=size, patch_size=encoder.patch_size, 
            device=device, criterion=criterion, metric=metric, 
            save_predictions=False, save_dir=None, num_objects_map=None
        )
...
if __name__ == "__main__":
    ...
    test(test_loader, encoder, decoder, model_type,size, encoder.patch_size, device, criterion, metric, 
        save_predictions=True, save_dir=predictions_path, num_objects_map=None
    )